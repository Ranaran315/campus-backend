...
export class NotificationsGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleConnection(client: Socket) {
        try {
            ...
            // 更新日志，包含用户名
            this.logger.log(`用户 ${username} (ID: ${userId}) 已连接。当前在线用户数: ${this.connectedUsers.size}`);
            
            // 发送连接成功消息
            client.emit('connected', { status: 'success', message: '成功连接到通知服务' });
        } catch (error) {
        ...
        try {
            ...
            if (userId && this.connectedUsers.has(userId)) {
                ...
                if (this.connectedUsers.get(userId) === client.id) {
                    ...
                    // 更新日志，包含用户名
                    this.logger.log(`用户 ${username} (ID: ${userId}) 已断开连接。当前在线用户数: ${this.connectedUsers.size}`);
                } else {
                     this.logger.warn(`用户 ${username} (ID: ${userId}) 的旧连接尝试断开，但当前连接ID不匹配。`);
                }
            } else {
                ...
                for (const [uid, socketId] of this.connectedUsers.entries()) {
                    if (socketId === client.id) {
                        ...
                        this.logger.log(`用户 (ID: ${uid}) 通过 Socket ID ${client.id} 断开连接。当前在线用户数: ${this.connectedUsers.size}`);
                        break;
                    }
                }
            }
    ...
    // 发送好友请求通知
    sendFriendRequestNotification(userId: string, requestData: FriendRequestNotificationData): boolean {
        try {
            const socketId = this.connectedUsers.get(userId);
            if (socketId) {
                this.server.to(socketId).emit('newFriendRequest', requestData);
                this.logger.log(`已向用户ID ${userId} 发送好友请求通知 (来自: ${requestData.sender.username})`);
                return true;
            } else {
                this.logger.log(`用户ID ${userId} 不在线，未发送好友请求通知 (来自: ${requestData.sender.username})`);
                return false;
            }
        } catch (error) {

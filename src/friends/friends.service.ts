...
export class FriendsService {
  ...
  async sendFriendRequest(senderId: string, receiverId: string, message?: string): Promise<any> {
    ...
    // 获取发送者信息并发送实时通知
    try {
      const senderInfo = await this.usersService.findOne(senderId) as unknown as UserInfo;
      ...
      if (senderInfo) {
        ...
        // 发送WebSocket通知
        this.notificationsGateway.sendFriendRequestNotification(receiverId, notificationData);
      }
    } catch (error) {
